function  [x,f,time]= GRPDA_L_new(A,B,b,a,tau,sigma,kesai)
    [rA,cA]=size(A);
    x = zeros(cA,1);
    p = zeros(rA,1);
    q = zeros(size(B,1),1);
    ...
    while time(1,end)<50
        ...
        while 2>1
            ...
            sita=tau/taupre;
            if sqrt(tau*sigma)*norm([Ax;Bx]-[Axpre;Bxpre],2) <=0.99*sqrt((2*a*a-6*a+4-sita)*sita/(1-a)/(a*a-3*a+2))*norm((x-xpre),2)
                break
            end
            tau=tau*0.7;
            sigma=sigma*0.7;
